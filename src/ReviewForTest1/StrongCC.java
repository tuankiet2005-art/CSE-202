...
public class StrongCC {
    ...
    public static void main(String[] args) throws IOException {
        ...
        Vertex[] graphCC = new Vertex[n];

        for (int i = 0; i < n; i++) {
            graph[i] = new Vertex(i);
            graphCC[i] = new Vertex(i);
        }
        ...
        for (int i = 0; i < m; i++) {
            ...
            graphCC[v].addAdj(graphCC[u]);
        }
        
        bfs(graph[b]);
        boolean canAReachB = graph[a].visited;
        
        bfs(graphCC[a]);
        boolean canBReachA = graphCC[b].visited;

        if (canAReachB && canBReachA) {
