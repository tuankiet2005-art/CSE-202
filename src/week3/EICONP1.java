...
public class EICONP1 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (Vertex vertex : graph) {
            Collections.sort(vertex.adjecentVertex, (s1,s2)->Integer.compare(s1.id, s2.id));
        }
        ...
        for (Vertex vertex : graph) {
            if(!vertex.visited){
                ...
                size = 0;
                dfs(vertex);
                sb.append(current).append(" ").append(size).append("\n");
            }
        }
    ...
    static void dfs(Vertex v){
        v.visited = true;
        size++;
        for (Vertex adjVertex : v.adjecentVertex) {
